document.addEventListener('DOMContentLoaded', function() {
    ...
    const binds = ["☆４キャラ武器", "回復禁止", "恒常☆５縛り", "国縛り", "モノ元素縛り", "武器種縛り", "誕生月", "キャラルーレット", "キャラ武器ルーレット" /* 他の縛りは省略または必要に応じて追加 */ ];
    ...
    // サブルーレットの選択肢を生成
    const subRoulettes = {
        "国縛り": [...new Set(characters.map(c => c.country))],
        "モノ元素縛り": [...new Set(characters.filter(c => c.element !== "その他").map(c => c.element))],
        "武器種縛り": [...new Set(characters.map(c => c.weapon))],
        "誕生月": [...new Set(characters.filter(c => c.birth_month !== "その他").map(c => c.birth_month))],
    };
    ...
    function startRoulette(type) {
        ...
        if (type === 'all' || type === 'boss') {
            ...
            items = binds.filter(b => !results.common.includes(b) && !Object.values(results.players).some(p => p[b]));
        }
        drawRoulette();
    }
    ...
    function setupRouletteForBind(bindName) {
        ...
        if (subRoulettes[bindName]) {
            currentRoulette = 'sub';
            items = subRoulettes[bindName];
        } else if (bindName === "キャラルーレット" || bindName === "キャラ武器ルーレット") {
            currentRoulette = 'character';
            items = getFilteredCharacters().map(c => c.name);
        } else { // 共通縛り
            results.common.push(bindName);
            proceedToNext();
            return;
        }
        showScreen('rouletteScreen');
    ...
    function getFilteredCharacters() {
        ...
        for (const bindName in playerBinds) {
            const value = playerBinds[bindName];
            if (!value) continue;

            switch(bindName) {
                case "国縛り":
                    filtered = filtered.filter(c => c.country === value);
                    break;
                case "モノ元素縛り":
                    filtered = filtered.filter(c => c.element === value);
                    break;
                case "武器種縛り":
                    filtered = filtered.filter(c => c.weapon === value);
                    break;
                case "誕生月":
                    filtered = filtered.filter(c => c.birth_month === value);
                    break;
            }
        }
        
        // 共通縛りを適用
        if (results.common.includes("恒常☆５縛り")) {
            filtered = filtered.filter(c => c.rarity.includes('恒常☆５'));
        }
        if (results.common.includes("☆４キャラ武器")) {
            filtered = filtered.filter(c => c.rarity.includes('☆４'));
        }
    ...
    function spinRoulette() { /* 変更なし */ }
    function animate() { /* 変更なし */ }
    function stopRoulette() { /* 変更なし */ }
    function showPopup(text) { /* 変更なし */ }
    ...
    function processResult() {
        if (currentRoulette === 'boss') {
            ...
            const bindName = currentBindName;
    ...
    function proceedToNext() {
        if (mode === 'selected') {
            ...
            if (results.common.length + totalPlayerBinds < bindCount) {
                currentRoulette = 'bind';
                items = binds.filter(b => !results.common.includes(b) && !results.players[0][b]);
                drawRoulette();
            } else {
                showResults();
            }
    ...
    function notOwned() {
        // '持っていない'機能はキャラクター選択時にのみ有効
        if(currentRoulette === 'character' || currentRoulette === 'weapon') {
            const excludedName = lastResult;
            items = items.filter(item => item !== excludedName);
            if (items.length === 0) {
                alert("候補のキャラクターがいなくなりました！");
                proceedToNext(); // 次のステップに進むか、結果表示
                return;
            }
            drawRoulette();
        }
    ...
    // ルーレット描画・回転の省略されたコードをここに展開
    const drawRoulette_hidden = () => {
        if (!items || items.length === 0) {
            ctx.clearRect(0, 0, canvas.width, canvas.height);
            ctx.fillStyle = '#000';
            ctx.font = '20px Arial';
    ...
    drawRoulette = drawRoulette_hidden;
    spinRoulette = () => {
